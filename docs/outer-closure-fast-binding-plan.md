...
- [x] Add runtime counters on `Runtime` (`fastPathHits`, `slowFallbacks`) plus REPL helpers (`__resetClosureStats`, `__closureStats`) so fast-path hits and guard-driven fallbacks stay observable during development.【F:src/NuXJS.h†L1168-L1204】【F:src/NuXJS.cpp†L2611-L2699】【F:tools/NuXJSREPL.cpp†L195-L211】
