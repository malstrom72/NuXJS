...
`with` temporarily installs an object environment ahead of the lexical frames that closures normally traverse.【F:docs/specs/ECMA-262 3.md†L3300-L3327】 Even if the compiler has already proven that an identifier resolves to a parent slot, the `WithScope` object must get first refusal at runtime. The existing implementation acknowledges this by incrementing `withScopeCounter` while the statement is parsed, preventing any locals declared within the block from taking the fast path, and by making `WithScope::resolveCapturedBinding` bail out so the interpreter reverts to hashed name lookup when a closure executes under a `with` guard.【F:src/NuXJS.cpp†L4240-L4249】【F:src/NuXJS.cpp†L2361-L2364】【F:src/NuXJS.cpp†L4032-L4040】 Failing to honour that ordering yields observably wrong behaviour. For example:
...
outer({ x: 42 }); // must return 42, not 1
```

The spec requires the property `x` on the `with` object to mask the outer slot while the statement executes. A pre-bound `(depth, slot)` operand would incorrectly skip the object and always return `1` unless the compiler blocks the optimization whenever a later `with` could intervene.

`catch` introduces a similar obstacle. Entering a `catch` clause synthesizes an object, binds the exception name as a property, and pushes that object onto the front of the scope chain for the duration of the handler.【F:docs/specs/ECMA-262 3.md†L3436-L3483】 NuXJS mirrors that behaviour by storing a `CATCH_PARAMETER` sentinel in `nameIndexes` so the identifier never upgrades to a slot binding, then restoring the original entry once the block ends.【F:src/NuXJS.cpp†L4505-L4533】 Consider:
...
outer(); // must return 2, not 1
```

The catch parameter shadows the outer `x` while the handler runs, so the closure has to observe the freshly bound exception value. If we emitted a direct slot operand for `inner` before seeing the later `catch (x)`, the generated bytecode would keep reading the `1` stored in the parent frame and violate ES3 semantics. Ensuring that `recordCapturedBinding` stops once it encounters a `CATCH_PARAMETER` entry keeps the interpreter on the safe named path when an exception handler intervenes, and `CatchScope` simply forwards resolution so the parent frame continues to see the handler object at the front of the chain.【F:src/NuXJS.cpp†L3891-L3925】【F:src/NuXJS.cpp†L2322-L2337】【F:src/NuXJS.cpp†L4516-L4533】 These examples illustrate why dynamic-scope guards must cover more than direct `eval`: both `with` and `catch` can legally reshuffle identifier resolution after nested closures have been parsed.
