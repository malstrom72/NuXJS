...
The minimal implementation drops per-capture diagnostics entirely; when a guard disallows slot emission, the compiler simply falls back to a named opcode without recording auxiliary metadata. Runtime counters remain available for observing fast and slow paths.
