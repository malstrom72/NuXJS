...
###### Preferred operand-only strategy

Among the guard-visibility options that let us delete the captured-binding array outright (solutions 1, 2, 5, 6, 7, 8, and 9), the guard-triggered recompilation handshake (solution 8) remains the most attractive path. It compiles the vast majority of functions only once, yet the rare recompilation run happens after the parent statement list has scanned every dynamic-scope hazard and hoisted declaration, so the regenerated bytecode can safely encode `(ancestorDistance, slotOffset)` operands without auxiliary metadata.【F:docs/outer-closure-fast-binding.md†L108-L199】【F:src/NuXJS.cpp†L3876-L3889】【F:src/NuXJS.cpp†L4085-L4125】 Because recompilation only fires when the guard actually flips, this strategy does not automatically expose locals declared later in the outer body; matching that capability would require augmenting the plan with a dedicated hoisting pre-pass or by always deferring nested compilation. Even so, the approach avoids the steady-state overhead of token buffering, placeholder opcodes, or dual-path emission.【F:docs/outer-closure-fast-binding.md†L108-L205】 In short, solution 8 gives us operand-only closures with the least long-term complexity while respecting the ES3 guard requirements already enforced in the parser and runtime.
