...
The `fails` manifest now lists eight ES3 Test262 cases across the Object, RegExp, and String built-ins.【F:fails†L1-L47】
...
5. **`built-ins/String/prototype/replace/S15.5.4.11_A3_T1`**, **`…_A3_T2`**, **`…_A3_T3`**
   ...
   **NuXJS diagnosis:** When encountering `$11`, `$12`, etc., the parser consumes both digits even when the two-digit capture index exceeds the available capture count, producing `$1115` instead of `x115`. The loop in `replaceFunction` never pushes the unused second digit back into the literal output.【F:src/stdlib.js†L493-L514】
