...
The `fails` manifest now lists three ES3 Test262 cases across the Array and String built-ins.【F:fails†L1-L14】
...
1. **`built-ins/String/prototype/replace/S15.5.4.11_A12`**
   ...
   **NuXJS diagnosis:** Borrowing `replace` with `this === undefined` should stringify the original receiver, producing the base string `"undefined"`. NuXJS instead routes the call through `support.callWithArgs`, which converts the `thisArg` with `Value::toObjectOrNull`. Passing `undefined` down that path returns `0`, so when the interpreter enters the JavaScript body it replaces the missing pointer with the global object.【F:src/NuXJS.cpp†L4792-L4816】【F:src/NuXJS.cpp†L2312-L2320】 The function’s first line then executes `var s, sLength = (s = str(this)).length` against the substituted global object, causing `str(this)` to emit `"[object Object]"` and the overall replacement to return `"[object Object]"` rather than `"unDefineD"`.【F:src/stdlib.js†L486-L541】
