...
========
	...
	* FIXED 2026-09-25 (es5 only, found 2026-09-19, review) 8.12.5: an array element store never consulted the prototype chain, so an inherited setter never ran, an inherited read-only element was overwritten, and a strict store never threw. JSArray::updateOwnProperty shared setOwnPropertyInternal with setOwnProperty, so the VM's pre-[[CanPut]] fast path created the element and reported success, standing in for 8.12.5 step 3 while doing step 6.
		- updateOwnProperty now refuses an index outside the dense range, so the store falls through to putThrough and 8.12.4 walks the chain. An existing dense element still updates in place, being own data that step 2 settles without the chain. tests/es5/arrayElementCanPut.io pins twelve shapes against V8; tests/unconforming/readOnlyNumericProps.io, which blessed the overwrite, moved to tests/es3only/ where it is still true.
		- the walk was cheap to add and expensive to run: a numeric key past the cached -1000..1000 range allocated a heap string at each prototype just to miss, 84% of the cost measured. Table now keeps a sticky bit set by insert on any digit-first key, and JSObject::getOwnProperty answers NONEXISTENT for an array-index number without stringifying when it is clear. Semantics-preserving for every caller: a clear bit proves the table holds no index, a stale one is only slower. JSArray::getOwnProperty also stops materializing an empty completeObject to miss in.
		- the refusal is gated on whether Array.prototype or Object.prototype may own an index, asked through a new Object::mayOwnIndexProperty() that each class answers for its own storage (conservatively true by default, from E's bit on JSObject, dense range plus table on JSArray, always true on StringWrapper). JSArray::getPrototype hardcodes the chain and ES5 has no setPrototypeOf, so it is one question for the whole heap, the shape of V8's NoElementsProtector. While both answer false the old path is exact, since [[CanPut]] could only refuse on an index the chain holds; it is derived on each append rather than cached, so there is no separate state to keep in step. If setPrototypeOf or __proto__ ever arrive this stops being one question, and the gate must go.
		- measured against a true es5 build of the parent (NOT output/NuXJS.exe, which build.cmd leaves as the es3 binary): bigArray, lz4_bm_1 and pure append loops are all within a timer tick of the parent. The fix alone was +53% on bigArray and +180% on lz4_bm_1; E alone brought that to +24%, the dispatch of the walk; the gate removes the rest. E still earns its place for a program that does put an index on a prototype, where it holds appends at +24% instead of +180%.

	* FIXED 2026-09-25 (es5 only, found 2026-09-19, review) 15.4.5.1: Object.getOwnPropertyNames on an array could report the same index twice, a plain store shadowing an element defineProperty had put in the sparse table. Gone with the fix above, the store now updating that element instead; arrayElementCanPut.io pins it.
