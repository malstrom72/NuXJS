...
======
	...
	* PRE-EXISTING (es3 and es5 alike, so a fix moves the es3 binary unless #if-guarded), found 2026-08-02 while testing the above: the comma operator yields a *reference* where 11.14 says it yields a value. `case COMMA` in postOperate is `xr = discard(xr); xr = operand(op);` - it drops the left operand and hands the right one's ExpressionResult straight back, so `(0, eval)` is still `NAMED("eval")`. 11.14 evaluates both operands through GetValue and returns that value, so the result cannot be a Reference at all. Two consequences, one root cause:
		- `(0, eval)(src)` runs as a DIRECT eval and sees the caller's scope: `(function () { var s = 1; return (0, eval)("typeof s"); })()` gives "number", V8 gives "undefined". 15.1.2.1.1 only calls it direct when the callee expression evaluates to a Reference named "eval" with an environment record as its base, and a comma result is not a Reference. This is the canonical "run this in global scope" idiom, so it is the one that matters. Note `(eval)(src)` IS direct in both engines and must stay that way: 11.1.6 parentheses preserve the Reference, which is why the GROUP case passing xr through is correct and only COMMA is wrong.
		- `typeof (0, undeclaredX)` answers "undefined" where V8 throws ReferenceError, because a NAMED result routes typeof through its no-throw path. GetValue on an unresolvable reference is what should throw first.
		- likely fix: makeRValue() the right operand in `case COMMA`. Cheap, but it is shared es3 code and the ES3 binary gate applies, and typeof/delete both read NAMED, so check those two before landing.
