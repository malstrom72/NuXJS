...
======
	...
	* two PRE-EXISTING indirect-eval scope leaks, found 2026-08-02 while testing the above. Both are in es3 as well, so a fix moves the es3 binary unless #if-guarded. 10.4.2 / 15.1.2.1.1 say an indirect eval runs as *global* code:
		- `(0, eval)('typeof someLocal')` sees the caller's locals; V8 says undefined. The comma operator hands back the last operand's ExpressionResult unchanged, so `(0, eval)` is still `NAMED("eval")` when the FUNCTION_CALL case tests for it and CALL_EVAL_OP is emitted. This is the canonical indirect-eval idiom, so it is the more serious of the two. Likely fix: the GROUP / comma paths should demote a NAMED result to PUSHED, but check what else reads NAMED through a group first.
		- `var g = eval; g('typeof arguments')` answers "object" where V8 says undefined, even though the same call correctly cannot see locals or parameters. So the scope chain is right and something resolves `arguments` anyway; worth finding out what before assuming it is the same bug as above.
