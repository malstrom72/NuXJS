...
========
	...
	* FIXED 2026-09-25 (es5 only, found 2026-09-19, review) 8.12.5: an array element store never consulted the prototype chain, so an inherited setter never ran, an inherited read-only element was overwritten, and a strict store never threw. JSArray::updateOwnProperty shared setOwnPropertyInternal with setOwnProperty, so the VM's pre-[[CanPut]] fast path created the element and reported success, standing in for 8.12.5 step 3 while doing step 6.
		...
		- measured against a true es5 build of the parent (NOT output/NuXJS.exe, which build.cmd leaves as the es3 binary): bigArray 1.17 -> 1.13s, unchanged; pure append loops and lz4_bm_1 about +24%, the dispatch of three getOwnProperty and two getPrototype calls per append. The plain fix without the bit was +53% and +180%.
		- the remaining +24% is what a global protector would recover: JSArray::getPrototype hardcodes the chain and ES5 has no setPrototypeOf, so whether Array.prototype or Object.prototype holds an accessor or read-only index is one question for the whole heap (V8's NoElementsProtector). Only those two kinds are observable - an inherited writable element changes nothing (8.12.4 step 8.2) - so ordinary Array.prototype[i] = v need not trip it. Not done; measure before adding it.
