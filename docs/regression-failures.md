...
- [ ] `numberConversionMania2.io` – `ToNumber` must round decimal strings to the nearest IEEE-754 double so converting through `Number` preserves the shortest `toString` form. The current mismatch reproduces even with the unmodified `parseDouble` implementation, so a different stage of the conversion pipeline is likely responsible. 【F:docs/specs/ECMA-262 5.1.md†L2236-L2244】【F:docs/specs/ECMA-262 5.1.md†L3132-L3135】
