...
// and NaN give nothing, and an out-of-range one wraps rather than being materialized - which used to kill the
// interpreter with an allocation failure the try/catch could not catch.
> var LENS = [2.5, 1.9, 0, -3, NaN, Infinity, -Infinity, 4294967295, 4294967296, 4294967297, 1e21], out = [];
> for (var k = 0; k < LENS.length; ++k) {
>	var L = LENS[k], plain = { 0: "a", 1: "b", 2: "c" }, acc = { 0: "a", 1: "b", 2: "c" };
>	plain.length = L;
>	Object.defineProperty(acc, "length", { get: function () { return L } });	// called before L moves on
>	var a = f.apply(null, plain), b = f.apply(null, acc);
>	out.push(a === b ? a : "MISMATCH " + a + "/" + b);
> }
> print(out.join(" "))
< 2:a,b 1:a 0: 0: 0: 0: 0: 0: 0: 1:a 0:
-
