...
"return a.tan(+G)})});function em(name,prototype){return function(message){var aG;a.defineProperty(aG=a.createWrapper("
"\"Error\",name,prototype),\"message\",(message!==void 0?P(message):''),false,true,false);return aG}};(function(){var e"
"n=[\"Error\",\"EvalError\",\"RangeError\",\"ReferenceError\",\"SyntaxError\",\"TypeError\",\"URIError\"];for(var u=en."
"length;--u>=0;){var C,au,X;a.defineProperty(b,C=en[u],au=em(C,X=a.prototypes[C]),false,true,false);au.name=C;Q(au,{don"
"tEnum:true,readOnly:true,dontDelete:true},{prototype:X});Q(X,{dontEnum:true},{constructor:au});X.name=C}Q(Error.protot"
"ype,{dontEnum:true},{message:'',toString:c(function toString(){return(this.name===void 0?\"Error\":this.name)+(this.me"
"ssage?(\": \"+this.message):'')})});l=SyntaxError;m=RangeError;n=TypeError})();var eo={'\\\\':\"\\\\\\\\\",'\"':\"\\\\"
"\\\"\",'\\b':\"\\\\b\",'\\f':\"\\\\f\",'\\n':\"\\\\n\",'\\r':\"\\\\r\",'\\t':\"\\\\t\"};var ep=61;Q(JSON,{dontEnum:tru"
"e},{stringify:c(function stringify(ae,eq,er){var es=[],et=(typeof eq===\"function\"?eq:null),eu='',ev;if(i(eq,\"class"
"\")===\"Array\"){ev={};for(var u=eq.length;--u>=0;)ev[eq[u]]=true}if(typeof er===\"number\"||(typeof er===\"object\"&&"
"i(er,\"class\")===\"Number\")){er=+er;for(var u=(er>10?10:er);--u>=0;)eu+=' '}else if(typeof er===\"string\"||(typeof "
"er===\"object\"&&i(er,\"class\")===\"String\")){eu=$sub(P(er),0,10)}function ew(B){var H='\"',ay=B.length;for(var u=0;"
"u<ay;++u){var aU=B[u],ex;H+=((ex=eo[aU])?ex:((aU>=' '&&aU<='~')?aU:\"\\\\u\"+a1(ad(k(aU,0),16),4)))}return H+'\"'}func"
"tion a7(ef,ey,ez){var ae;if((ae=ey[ef])&&typeof ae===\"object\"&&typeof ae.toJSON===\"function\")ae=ae.toJSON(ef);if(e"
"t)ae=j(et,ey,[ef,ae]);var eA=(eu?'\\n'+ez:'');if(typeof ae===\"object\"){switch(i(ae,\"class\")){case\"Number\":ae=+ae"
";break;case\"String\":ae=P(ae);break;case\"Boolean\":ae=i(ae,\"value\");break}}switch(typeof ae){case\"object\":{if(!a"
"e)return\"null\";var ay,B=new x,G;for(var u=(ay=es.length);--u>=0;){if(es[u]===ae)throw n(\"Cannot convert circular st"
"ructure to JSON\")}if(ay>ep)throw n(\"Structure too deeply nested for JSON conversion\");es[ay]=ae;if(i(ae,\"class\")="
"==\"Array\"){ay=ae.length;for(var u=0;u<ay;++u){B.A((u==0?'[':',')+eA+eu+(a7(u,ae,ez+eu)||\"null\"))}B.A(ay==0?\"[]\":"
"eA+']')}else{var eB=false;for(var cL in(ev?ev:ae)){if(a.hasOwnProperty(ae,cL)){if(G=a7(cL,ae,ez+eu)){B.A((eB?',':'{')+"
"eA+eu+ew(cL)+(eu?\": \":':')+G);eB=true}}}B.A(eB?eA+'}':\"{}\")}--es.length;return B.D()}case\"string\":return ew(ae);"
"case\"number\":return(e(ae)?P(ae):\"null\");case\"boolean\":return P(ae)}}return a7('',{'':ae},'')}),parse:c(function "
"parse(eC,eD){var eE=0;function er(H,X){var aU;while((aU=H[X])===' '||aU==='\\t'||aU==='\\r'||aU==='\\n')++X;return X}f"
"unction eF(H,X){var aU;if(H[X]==='-')++X;if((aU=H[X])==='0')aU=H[++X];else if(aU>='1'&&aU<='9')do{aU=H[++X]}while(aU>="
"'0'&&aU<='9');else return;if(aU==='.'){if((aU=H[++X])<'0'||aU>'9')return;while((aU=H[++X])>='0'&&aU<='9')}if(aU==='e'|"
"|aU==='E'){if((aU=H[++X])==='-'||aU==='+')aU=H[++X];if(aU<'0'||aU>'9')return;while((aU=H[++X])>='0'&&aU<='9')}return X"
"}function a7(H,X){var aU;if(H[X]!=='\"')return;++X;while((aU=H[X])!=='\"'&&aU>=' '){if(aU==='\\\\'){switch(H[++X]){def"
"ault:return;case'\"':case'/':case'\\\\':case'b':case'f':case'n':case'r':case't':break;case'u':for(var u=4;--u>=0;)if(!"
"(((aU=H[++X])>='0'&&aU<='9')||(aU>='a'&&aU<='f')||(aU>='A'&&aU<='F')))return;break}}++X}if(aU==='\"')return++X}functio"
"n eG(H,X){if($match(H,X,\"true\")||$match(H,X,\"null\"))return X+4;else if($match(H,X,\"false\"))return X+5}function R"
"(H,X){if(eE>ep)throw n(\"Structure too deeply nested for JSON conversion\");++eE;try{var aU,eH=false,eI=H[X],eJ=(eI==="
"'['?']':'}');X=er(H,++X);while((aU=H[X])!==eJ&&aU){if(eH){if(aU!==',')return;X=er(H,++X)}if(eI==='{'){if(!(X=a7(H,X))|"
"|H[X=er(H,X)]!==':')return;X=er(H,++X)}if(!(eK=eL[H[X]])||!(X=eK(H,X)))return;X=er(H,X);eH=true}if(aU===eJ)return++X}f"
"inally{--eE}}var eL={'{':R,'[':R,'\"':a7,'t':eG,'f':eG,'n':eG,'-':eF,'0':eF,'1':eF,'2':eF,'3':eF,'4':eF,'5':eF,'6':eF,"
"'7':eF,'8':eF,'9':eF};eC=P(eC);var X,eK;if((eK=eL[eC[X=er(eC,0)]])&&(X=eK(eC,X))&&er(eC,X)===eC.length){var ae=eval('("
"'+eC+')');if(typeof eD===\"function\"){function eM(ey,ef){var cL,G,J;if(typeof(J=ey[ef])===\"object\"&&J){for(cL in J)"
"{if(a.hasOwnProperty(J,cL)){if((G=eM(J,cL))!==void 0)J[cL]=G;else delete J[cL]}}}return j(eD,ey,[ef,J])}ae=eM({\"\":ae"
"},\"\")}return ae}throw l(\"Error parsing JSON\")})});Q(Array,{dontEnum:true},{isArray:c(function isArray(J){return i("
"J,\"class\")===\"Array\"})});Q(Object,{dontEnum:true},{defineProperty:c(function defineProperty(J,X,cb){a.defineProper"
"ty(J,P(X),cb.value,!cb.writable,!cb.enumerable,!cb.configurable)}),getPrototypeOf:c(function getPrototypeOf(J){return "
