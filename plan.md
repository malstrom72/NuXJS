...
- [ ] **Step 8 – Update the embedder surface.** Search for any code that advertises `String::toUTF8String()` as returning strict Unicode (documentation in `docs/Embedding Guide.md`, API comments, public headers). Amend the wording to clarify that the result uses WTF-8 to guarantee round-tripping of ECMAScript strings.
- [ ] **Step 9 – Cross-platform verification.** Build and run both the Unix (`build.sh`) and Windows (`build.cmd` under CI) targets because the code touches shared runtime pieces. Pay special attention to the MSVC build, ensuring `_MSC_VER`-gated paths in `String::toUTF8String()` compile with the new helper.
- [ ] **Step 10 – Regression testing.** Augment the `tests/regression/loneSurrogateErrorMessageCrash.io` added earlier with positive cases that ensure the engine no longer asserts and that `String.fromCharCode(0xD800).toString()` round-trips. Also add a fuzzer seed that concatenates mixed valid and invalid surrogate pairs to verify we never crash while building error messages.
